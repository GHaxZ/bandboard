...
// In-flight requests are deduped so two components asking for the same track
// concurrently share one fetch+decode.
const inFlight = new Map<string, Promise<{ peaks: Float32Array; duration: number } | null>>();
...
export function getWaveformPeaks(
  ...
  const task = decodePeaks(srcUrl)
    .then((result) => {
      inFlight.delete(trackId);
      if (result) cacheSet(trackId, result);
      return result;
    })
    .catch(() => {
      inFlight.delete(trackId);
